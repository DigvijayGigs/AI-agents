...
# Define the 3x3 environment( Rooms )
environment = {
    "Room1": "Clean",
...
# Mapping for grid positions ( Room Numbers )
room_positions = {
    "Room1": (0, 2),  # Top-left
    "Room2": (1, 2),  
    "Room3": (2, 2),  
...
def draw_environment(env, agent_pos, step):
    ...
    for room, pos in room_positions.items():  
        ...
        ax.add_patch(rect)
        ax.text(x + 0.5, y + 0.5, room, ha='center', va='center', color='black', fontsize=10) # "Room1"

    # Draw agent
    agent_x, agent_y = room_positions[rooms[agent_pos]] # Finds the agent's current position.
    agent_patch = patches.Circle((agent_x + 0.5, agent_y + 0.5), 0.3, color='white') #blue circle
...
plt.ioff() 
print("Simulation complete, all rooms are cleaned now.") 
